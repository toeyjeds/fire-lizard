...
- `docker-compose.yml` — SQL Server, SMTP relay (MailHog), both backend services, and the frontend
...
4. Externalize all configuration (SQL Server, SMTP, Azure AAD, rate limit) via environment variables; never hard-code secrets.
...
- SQL Server and SMTP containers start successfully.
